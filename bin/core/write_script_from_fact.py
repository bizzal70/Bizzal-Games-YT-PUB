...
_VAGUE_FILLER_RE = re.compile(
    ...
    # Overused hook skeletons flagged twice by the content reviewer
    # (2026-08-20, 2026-08-28): reused near-verbatim across many different
    # subjects until the pattern itself became the giveaway.
    r"|surprises (?:many|players|parties|GMs|DMs)"
    r"|catches (?:players|parties) off guard"
    r"|(?:often )?gets? misused for",
    re.IGNORECASE,
)
...
def maybe_ai_polish_script(atom: dict, fact: dict, style: dict, script: dict) -> dict:
    ...
    prompt = {
        ...
        "rules": [
            ...
            "Hook: one dry declarative sentence, REQUIRED two-part shape: a short claim about what the mechanic does or how it surprises players, THEN an em dash (—), THEN the specific number/condition that confirms the claim. This is not optional decoration — the hook gets cut at the dash and the claim half alone becomes the video title, so if the claim half already gives away the specific number or full mechanic, the title has nothing left to reveal and nobody clicks. The claim half must raise a real question (how much? under what condition? how often?) that only the dash half answers. Never open with 'Players often'. Stated as fact, not a question. Do NOT recite a raw stat line as the claim half. BAD (flat fact, no claim/reveal split at all — this is the most common failure): 'Alchemist's Supplies cost 50 GP and weigh 8 pounds.' BAD (dash present but the claim half already spoils the payoff): 'Awakened Tree has a Strength score of 19 — its Slam deals 3d6 + 4.' GOOD (claim half raises a real question, dash half answers it): 'Awakened Tree's Slam hits harder than most parties expect at this level — 3d6 + 4 puts most low-AC builds in one-shot range.'",
